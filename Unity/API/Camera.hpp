...
namespace Unity
{
    ...
    public:
        float GetDepth()
        {
            if (!m_CameraFunctions.m_GetDepth) return 0.0f;
        ...
        {
            if (!m_CameraFunctions.m_SetFieldOfView) return;
        ...
        {
            if (!m_CameraFunctions.m_WorldToScreen) return;
    ...
    namespace Camera
    {
        ...
        CCamera* GetCurrent()
        {
            if (!m_CameraFunctions.m_GetCurrent) return nullptr;
            return reinterpret_cast<CCamera*(UNITY_CALLING_CONVENTION)(void*)>(m_CameraFunctions.m_GetCurrent)(nullptr);
        }

        CCamera* GetMain()
        {
            if (!m_CameraFunctions.m_GetMain) return nullptr;
            return reinterpret_cast<CCamera*(UNITY_CALLING_CONVENTION)(void*)>(m_CameraFunctions.m_GetMain)(nullptr);
        }
    }
